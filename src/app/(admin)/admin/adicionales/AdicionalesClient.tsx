...
import {
  ...
  updateAdicional,
  deleteAdicional,
  toggleAdicionalAvailability,
  getAdicionalUsage,
  reorderAdicionales,
} from "@/actions/adicionales";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      <div className="relative w-full max-w-md rounded-t-2xl sm:rounded-2xl bg-white p-6 shadow-elevated ring-1 ring-border sm:mx-4 animate-in">
        <h2 className="text-lg font-semibold text-text-main mb-5">Nuevo adicional</h2>
...
export function AdicionalesClient({
  adicionales,
  exchangeRate,
}: AdicionalesClientProps) {
  const [items, setItems] = useState(adicionales);
  ...
  const handleCreate = async (data: { name: string; priceUsdCents: number; isAvailable: boolean; sortOrder: number }) => {
    const result = await createAdicional(data);
    if (result.success) {
      setItems((prev) => [...prev, result.adicional!].sort((a, b) => a.sortOrder - b.sortOrder));
    }
  };

  const handleSave = async (id: string, data: { name: string; priceUsdCents: number }) => {
    const result = await updateAdicional(id, data);
    if (result.success) {
  ...
  const handleToggle = async (id: string, isAvailable: boolean) => {
    ...
    await toggleAdicionalAvailability(id, isAvailable);
  };

  const handleDeleteClick = async (id: string, name: string) => {
    const count = await getAdicionalUsage(id);
    if (count > 0) {
  ...
  const handleConfirmDelete = async () => {
    ...
    await deleteAdicional(deleteWarning.id);
  ...
  const handleDragEnd = async () => {
    ...
    await reorderAdicionales(reordered.map((i) => i.id));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-text-main">Adicionales</h1>
          <p className="text-sm text-text-muted">Gestiona los acompañamientos opcionales para los platos</p>
        </div>
        ...
        <CardContent className="p-0">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <p className="text-sm font-medium text-text-main">Sin adicionales</p>
      ...
      {deleteWarning && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-elevated ring-1 ring-border mx-4">
            <h3 className="text-lg font-semibold text-text-main mb-2">Adicional asignado</h3>
